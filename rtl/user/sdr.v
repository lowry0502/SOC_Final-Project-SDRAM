...
`endif             
    ...
    // Detect Read or Write command
            ...
            if (Read_enable || Command[0] == `WRITE) begin//Command[2] == `READ
                if(Read_enable && Burst_length_1 == 1'b1)begin//Command[2] == `READ
                    Bank <= Ba;//Bank_addr[2]
                    Col <= Addr;//Col_addr[2]
                ...
                else if(Read_enable && Burst_length_8 == 1'b1) begin
                    Bank <= Ba;//Bank_addr[2]
                    Col <= Addr;//Col_addr[2]
                    Col_brst <= addr_temp + Burst_counter;//Col_addr[2]
...
//                 */
    ...
    always@(posedge Sys_clk or negedge Rst_n)begin
        if(!Rst_n)begin
            ...
            else if (Burst_length_1 == 1'b1) begin
                if (Read_enable || Command[0] == `WRITE) begin//Command[2] == `READ
                    //Data_in_enable <= 1'b0;
                    //Data_out_enable <= 1'b1;
                    if(Read_enable) begin//Command[2] == `READ
